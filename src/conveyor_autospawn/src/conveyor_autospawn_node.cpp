...
ConveyorAutospawnNode::ConveyorAutospawnNode()
    ...
    std::string ConveyorAutospawnNode::load_box_urdf(){
        ...
        if(!in.is_open()) throw std::runtime_error("Cannot open URDF" + urdf_path);
    ...
    void ConveyorAutospawnNode::schedule_next_spawn(){
        ...
        timer_ = this->create_wall_timer(ms, std::bind(&ConveyorAutospawnNode::spawn_once, this));
    }
