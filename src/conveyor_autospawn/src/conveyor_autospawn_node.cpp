...
ConveyorAutospawnNode::ConveyorAutospawnNode()
    : Node("conveyor_autospawn_node"), rng_(std::random_device{}()), count_(0) {
  ...
  uni_interval_ =
      std::uniform_real_distribution<double>(min_interval_s_, max_interval_s_);
  uni_x_ = std::uniform_real_distribution<double>(x_min_, x_max_);
  conveyor_client_ =
      this->create_client<conveyorbelt_msgs::srv::ConveyorBeltControl>(
          "/CONVEYORPOWER");
  spawn_client_ =
  ...
  while (!conveyor_client_->wait_for_service(10s)) {
    if (!rclcpp::ok())
      throw std::runtime_error(
          "Interrupted while waiting for conveyor service");
  }

  while (!spawn_client_->wait_for_service(10s)) {
    if (!rclcpp::ok())
      throw std::runtime_error("Interrupted while waiting for spawn service");
  }
}

void ConveyorAutospawnNode::send_conveyor_speed(int power) {
  auto req =
      std::make_shared<conveyorbelt_msgs::srv::ConveyorBeltControl::Request>();
  req->power = power;
  conveyor_client_->async_send_request(req);
  RCLCPP_INFO(get_logger(), "Requested conveyor power %d", power);
}

std::unique_ptr<XMLDocument> ConveyorAutospawnNode::load_box_urdf() {
  const std::string pkg_share =
      ament_index_cpp::get_package_share_directory("conveyorbelt_gazebo");
...
void ConveyorAutospawnNode::schedule_next_spawn() {
  ...
  timer_ = this->create_wall_timer(ms, [this]() { this->spawn_once(); });
}
...
void ConveyorAutospawnNode::spawn_once() {
  ...
  spawn_client_->async_send_request(
      ...
      [this, self](
        ...
        if (!resp->success) {
          RCLCPP_WARN(this->get_logger(), "Spawn failed: %s",
